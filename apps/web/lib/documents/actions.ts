...
import { MAX_UPLOAD_SIZE_BYTES, PDF_SIGNATURE } from "@/lib/documents/config";
 ...
 * ADR 0008): independently re-downloads the object from Storage using this
 * same session-bound client (authorized by the same RLS policy that
 * authorized the upload), checks size and the PDF signature, computes
 * SHA-256 over the actual bytes, and only then calls the RPC that
 * atomically records the verification decision, registers the document,
...
export async function completeGuidelineUploadAction(uploadSessionId: string): Promise<UploadCompletionResult> {
  ...
  const { data: blob, error: downloadError } = await supabase.storage.from(session.storage_bucket).download(session.storage_path);
  ...
  if (downloadError || !blob) {
    ...
    } else if (sizeBytes > MAX_UPLOAD_SIZE_BYTES) {
    ...
    const signatureValid = buffer.subarray(0, PDF_SIGNATURE.length).toString("latin1") === PDF_SIGNATURE;
    detectedMediaType = signatureValid ? "application/pdf" : "application/octet-stream";
    if (!rejectionReason && !signatureValid) {
